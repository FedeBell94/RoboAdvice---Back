...
//    List<Portfolio> findByUserAndStartDateGroupByDateAndAssetClassId(User user, Date date);
    ...
    @org.springframework.data.jpa.repository.Query("SELECT SUM(p.value) AS sum,p.date as date FROM Portfolio p WHERE p.user = ?1 AND p.date > ?2 group by p.assetClass")
